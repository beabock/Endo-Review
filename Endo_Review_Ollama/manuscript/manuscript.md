...
- Fungal endophytes are frequently described as ubiquitous in the literature and reviews [@stone_overview_2000; @arnold_are_2000; @cosner_fungal_2025], but that claim has not been evaluated with a global, quantitative synthesis.
- This manuscript should be framed as a conservative test of the ubiquity claim and a quantitative map of where ecological inference is currently credible versus where evidence is missing.
- The key advance is a reproducible evidence-gap analysis linking host taxonomy, geography, and research capacity to define a priority sampling roadmap, while explicitly acknowledging that largely abstract-only mining will miss some relevant studies.
...
## Abstract (Outline)
- Background: Fungal endophytes are often described as ubiquitous, yet this has not been tested with a truly global synthesis [@stone_overview_2000; @arnold_are_2000; @cosner_fungal_2025].
...
6. Brief statement of scope limits (mostly metadata/abstract-driven evidence because of lack of open-access texts) and why this is still informative; this should be explicit up front because reviewers objected to the abstract-only design.

## Methods (Outline)

### Main Methods
1. Corpus assembly and scope
	- Abstracts were consolidated from Web of Science, Scopus, and PubMed, deduplicated, and filtered to a final analysis corpus.
	- Briefly report how many records had full text available through open access versus how many were abstract-only because of access barriers; in the current access audit, 3,068 records were full-text accessible and 15,682 were abstract-only (18,750 total).
	- State the main scope limits up front: this is an abstract-led evidence audit, not a full-text ecology review.
2. Metadata standardization
	- Country assignment, host taxonomy harmonization, biome mapping, tissue standardization, and publication-year enrichment.
	- High-level mention of GBIF Backbone taxonomic resolution and country standardization.
3. Core analyses
	- Geographic concentration and country-level bias.
	- GDP and latitude relationships.
	- Host taxonomic coverage and understudied-taxa prioritization.
	- Interaction / relationship-type summaries.
	- Ubiquity-claim detection in the full-text/abstract evidence base.
4. Reproducibility and validation
	- Brief statement that the pipeline is script-based, versioned, and checkpointed.
...
1. Corpus assembly details
	...
	- Scripts: [01_csv_cleanup.py](scripts/01_data_preproccessing/01_csv_cleanup.py), [api_pull_abstracts.R](scripts/01_data_preproccessing/api_pull_abstracts.R), [Combo_abstracts_pull2.R](scripts/01_data_preproccessing/Combo_abstracts_pull2.R), [merge_final_data.py](scripts/01_data_preproccessing/merge_final_data.py).
2. Taxonomic resolution and synonym handling
	...
	- Scripts: [taxa_synonym_resolution.py](scripts/02_taxa_resolution/taxa_synonym_resolution.py), [taxon_mapping.py](scripts/utils/taxon_mapping.py), [country_mapping.py](scripts/utils/country_mapping.py).
...
1. Evidence base for evaluating the ubiquity claim
	...
	- Report how many abstracts/papers explicitly state ubiquity or near-ubiquity claims, because this directly addresses the field assumption and the silent-axiom point from the old manuscript (CITATION - will have this analysis done soon).
...
4. Reliability and scope limits
	- Mostly abstract-level and indexing constraints.
	- Taxonomic and nomenclatural uncertainty handling.
	- Country-level aggregation caveat and biome context as partial remedy.
	- Add a short paragraph acknowledging the reviewer point that abstracts miss many taxa, countries, and methods, so the paper likely underestimates true coverage gaps rather than exaggerating them
	- Add a sentence or two on access limitations: 3,068 records were full-text accessible and 15,682 were abstract-only, which constrains extraction depth and makes the abstract-only component the dominant part of the evidence base. This can also be a quick critique on publishing behind paywalls. 
	- Consider a dedicated subsection on nomenclature and functional-group misclassification if the final manuscript includes those results. E.g. on mycorrhizae.
